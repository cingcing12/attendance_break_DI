...
const openConfirmModal = (config) => {
    modal.value = { ...modal.value, show: true, type: 'confirm', ...config };
};
  ...
  <div class="flex flex-col min-h-screen pb-32 md:pb-12 bg-[#050505]">
        ...
        <div class="glass-panel rounded-2xl px-6 py-3 max-w-[1600px] mx-auto flex flex-col md:flex-row items-center justify-between gap-4">
            ...
            <div class="flex items-center gap-6 w-full md:w-auto">
                ...
                <div class="flex flex-col">
                    ...
                    <span class="text-[10px] font-bold tracking-widest uppercase mt-0.5" 
                          :class="currentArea === 'A' ? 'text-cyan-400' : 'text-rose-400'">
                        AREA {{ currentArea }}
                    </span>
                </div>
            </div>
            ...
            <div class="hidden md:flex bg-black/40 p-1 rounded-xl border border-white/10 relative w-96 shadow-inner shrink-0 h-12">
                <div class="absolute top-1 bottom-1 w-[calc(50%-4px)] bg-white rounded-lg transition-all duration-300 shadow-md z-0"
                     :style="{ left: currentTab === 'staff' ? '4px' : 'calc(50%)' }">
                </div>
                <button @click="currentTab = 'staff'" 
                        class="relative z-10 w-1/2 h-full text-sm font-bold flex items-center justify-center gap-2 transition-colors duration-300"
                        :class="currentTab === 'staff' ? 'text-black' : 'text-slate-400 hover:text-white'">
                    <Search class="w-4 h-4" /> ស្វែងរក
                </button>
                <button @click="currentTab = 'active'" 
                        class="relative z-10 w-1/2 h-full text-sm font-bold flex items-center justify-center gap-2 transition-colors duration-300"
                        :class="currentTab === 'active' ? 'text-black' : 'text-slate-400 hover:text-white'">
            ...
            <div class="w-full md:w-80">
                <div class="flex items-center w-full bg-surface/50 border border-white/10 rounded-xl px-4 py-2.5 focus-within:border-cyan-500/50 focus-within:bg-surface transition-all">
                    ...
                    <input ref="searchInputRef"
                           ...
                           class="w-full bg-transparent border-none outline-none text-white font-khmer text-sm placeholder-slate-500 h-full py-1" 
    ...
    <main class="flex-grow px-4 max-w-[1600px] mx-auto w-full animate-fade-in">
        ...
        <div v-if="currentTab === 'staff'">
            ...
            <div v-else-if="filteredStaff.length === 0" class="text-center py-20 text-slate-500">
                No Staff Found
            </div>
            <div v-else class="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4 md:gap-6">
                <StaffCard 
                    ...
                    :isOnBreak="store.activeBreaks.some(b => b.id == person.id && !b.timeIn)"
                    :breakData="store.activeBreaks.find(b => b.id == person.id)"
        ...
        <div v-if="currentTab === 'active'">
            ...
            <div v-else class="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4 md:gap-6">
                <BreakCard 
                    v-for="item in filteredBreaks" 
                    :key="item.id" 
                    :item="item"
                    :now="now" 
                    :staffInfo="store.allStaff.find(s => s.id == item.id)"
                    @timeIn="handleTimeIn"
                />
            </div>
        </div>
    ...
    <nav class="md:hidden fixed bottom-6 left-1/2 -translate-x-1/2 bg-[#18181b]/95 backdrop-blur-xl border border-white/10 rounded-full px-2 py-2 flex gap-2 z-[100] shadow-[0_10px_40px_rgba(0,0,0,0.5)]">
        <button @click="currentTab = 'staff'" 
            class="flex items-center gap-2 px-6 py-3 rounded-full transition-all duration-300"
            :class="currentTab === 'staff' ? 'bg-white text-black' : 'text-slate-500 hover:text-slate-300'">
            <Search class="w-5 h-5" />
            <span v-if="currentTab === 'staff'" class="text-xs font-bold uppercase tracking-widest animate-fade-in">Search</span>
        </button>
        <button @click="currentTab = 'active'" 
            class="flex items-center gap-2 px-6 py-3 rounded-full transition-all duration-300"
            :class="currentTab === 'active' ? 'bg-white text-black' : 'text-slate-500 hover:text-slate-300'">
            <Clock class="w-5 h-5" />
            <span v-if="currentTab === 'active'" class="text-xs font-bold uppercase tracking-widest animate-fade-in">Active</span>
        </button>
    </nav>
